...
var Student = /** @class */ (function () {
	...
	Student.create = function (newstudent, result) {
		sql.query("INSERT INTO students SET ?", newstudent, function (err, res) {
			...
			console.log(
				"created student: ",
				__assign({ id: res.insertId }, newstudent)
			);
	...
	Student.findById = function (student, result) {
		sql.query(
			"SELECT * FROM students WHERE id = " + student,
			function (err, res) {
				if (err) {
					console.log("error: ", err);
	...
	Student.studentsIdsFromInstitution = function (institution_id, result) {
		sql.query(
			"SELECT * FROM module_student WHERE institution_id = " + institution_id,
			function (err, res) {
				if (err) {
					console.log("error: ", err);
					result(err, null);
...
exports.default = Student; // end of the class:
