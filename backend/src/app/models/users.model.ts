...
export default class User{
    ...
    static findById(user: any, result: any){
      sql.query(`SELECT * FROM users WHERE id = ${user}`, (err: Error, res: any) => {
        if (err) {
          console.log("error: ", err);
          result(err, null);
          return;
    ...
    static login(obj: any, result: any){
      sql.query(`SELECT * FROM users WHERE email = ${obj.email} AND password = ${obj.password}`, (err: Error, res: any) => {
        if (err) {
