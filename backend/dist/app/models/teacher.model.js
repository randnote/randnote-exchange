...
var Teacher = /** @class */ (function () {
    ...
    Teacher.create = function (newteacher, result) {
        sql.query("INSERT INTO teachers SET ?", newteacher, function (err, res) {
            ...
            console.log("created teacher: ", __assign({ id: res.insertId }, newteacher));
    ...
    Teacher.findById = function (teacher, result) {
        sql.query("SELECT * FROM teachers WHERE id = " + teacher, function (err, res) {
            if (err) {
                console.log("error: ", err);
                result(err, null);
                return;
    ...
    Teacher.login = function (obj, result) {
        sql.query("SELECT * FROM teachers WHERE staffnumber = " + obj.staffnumber + " AND password = " + obj.password, function (err, res) {
            if (err) {
                console.log("error: ", err);
                // result(err, null);
    ...
    Teacher.teachersFromInstitution = function (institution_id, result) {
        sql.query("SELECT * FROM teachers WHERE institution_id = " + institution_id, function (err, res) {
            if (err) {
                console.log("error: ", err);
                result(err, null);
                return;
